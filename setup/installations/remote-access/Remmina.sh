...
function main() {
  ...
  while true
  do
    local __software_name=Remmina

    # check code`s existence
    check_executable_exists remmina \
...
This script will install '${nocolorbold}${__software_name}${nocolor}':
  ...
  * Documentation: https://remmina.org
'
    ...
    sudo snap install remmina || break

    sudo snap connect remmina:avahi-observe :avahi-observe || break
    sudo snap connect remmina:cups-control :cups-control || break
    sudo snap connect remmina:mount-observe :mount-observe || break
    sudo snap connect remmina:password-manager-service :password-manager-service || break
