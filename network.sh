...
function network_get_data() {
  # wget -q -O - 'https://www.flickr.com/services/rest/?method=flickr.photos.search&
# api_key=b1f540b8e47778cb0590901651837659&text=motorbike&extras=url_c&per_page=25&page=3&format=json&nojsoncallback=1'
  ...
  case $__download_command in
    curl)
      [ ${__check_ssl_cert} -eq 1 ] \
        && __download_command=curl\ -L\ ${__url} \
        || __download_command=curl\ -L\ -k\ ${__url}
      ;;
    wget)
      [ ${__check_ssl_cert} -eq 1 ] \
